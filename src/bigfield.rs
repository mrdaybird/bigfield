...
#[macro_export]
macro_rules! create_special_modulus1 {
...
#[macro_export]
macro_rules! create_special_modulus2 {
    ($name:ident, $type: ty, $t:expr, $c: expr) => {
        pub struct $name;
        impl Modulus<$type> for $name {
            const M: $type = {
    ...
    ($name: ident, $type: ty, $modulus: expr) => {
        ...
        impl Modulus<$type> for $name {
            ...
            fn reduce(xt: ($type, $type)) -> $type {
                ...
                // mask = (1 << 320) - 1;
                let mask =
                    Concat::new(&BigInt::ZERO, &BigInt::pow_of_2(Word::BITS)).sub(&Concat::ONE);
...
impl<const N: usize, P: Modulus<BigInt<N>>> PrimeField<BigInt<N>, P> {
    const ZERO: PrimeField<BigInt<N>, P> = Self {
        value: BigInt::from_u64(0),
        phantom: PhantomData,
    };
    const ONE: PrimeField<BigInt<N>, P> = Self {
    ...
    where
        [(); N * 2]:,
    {
        self.pow(&(P::M.sub(&BigInt::<N>::ONE)).shr(1).0)
            .eq(&Self::ONE)
    }
...
#[cfg(test)]
mod tests {
    use crypto_bigint::modular::{ConstMontyForm, ConstMontyParams, Retrieve};
    use crypto_bigint::{impl_modulus, Encoding, U256, U512};
    use hex::ToHex;
    use hex_literal::hex;
    use rand::Rng;
    use rstest::rstest;
    ...
    create_special_modulus2!(L0, BigInt<4>, 252, hex!("0000000000000000000000000000000014def9dea2f79cd65812631a5cf5d3ed"));
    // create_modulus!(
    //     L0,
    //     BigInt<4>,
    ...
    fn test_concat_reduction() {
        ...
        for _i in 0..10000 {
            ...
            let expected1: [u8; 32] = F2::new(&U512::from_le_bytes(bytes))
                .retrieve()
                .to_le_bytes()[..32]
                .try_into()
                .unwrap();

            let expected2: [u8; 32] = F5::new(&U512::from_le_bytes(bytes))
                .retrieve()
                .to_le_bytes()[..32]
                .try_into()
                .unwrap();
